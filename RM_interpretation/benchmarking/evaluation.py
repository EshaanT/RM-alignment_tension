...
from transformers.utils import ExplicitEnum, add_end_docstrings, is_torch_available
...
def rm_acc(dataset:dict,rej_outputs:list,cho_outputs:list):  # noqa: ANN201, D103

    rej_index_dic=defaultdict(list)
    for item, idx in zip(rej_outputs, dataset['rejected2ids'], strict=True):
        rej_index_dic[idx].append(item["score"])
    rej_index_dic=dict(rej_index_dic)

    cho_index_dic=defaultdict(list)
    for item, idx in zip(cho_outputs, dataset['chosen2ids'], strict=True):
        cho_index_dic[idx].append(item["score"])
    cho_index_dic=dict(cho_index_dic)
    ...
    for idx in dataset['ids']:
        ...
        if min(cho_index_dic[idx])>max(rej_index_dic[idx]):
            correct_prefed.append(1)
        else:
            correct_prefed.append(0)
        score_pref.append((min(cho_index_dic[idx]),max(rej_index_dic[idx])))
    return correct_prefed,score_pref
